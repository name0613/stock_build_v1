...
| `TaiwanStockInstitutionalInvestorsBuySell` | equivalent raw verification/fallback | S1 equivalent, never double-counted | daily source date |
...
`FinMindClient.probe()` makes a real, server-side capability probe and writes sanitized `FINMIND_CAPABILITY_EVIDENCE.json`. The implementation does not assume SponsorPro. If a market-wide object is unavailable, `fetch_broker_stocks()` uses stock-by-stock async requests with a semaphore, rate limiter, retry/backoff/jitter, Retry-After, request metrics and a JSON checkpoint that allows resume after restart.
