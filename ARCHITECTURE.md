...
Broker completeness is bound to `finmind-dedicated-stock-session-report-v1`: the dedicated endpoint must return one requested stock/session, every row must have broker ID and non-null buy/sell, and explicit incomplete pagination fails closed. Only then may an omitted branch mean zero activity. Holding completeness is bound to `finmind-holding-shares-level-v1`: all 15 canonical thresholds must occur exactly once with non-null percent, people and shares; unknown, duplicate or missing buckets fail closed.
