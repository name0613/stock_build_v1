...
test("favorite star updates immediately before the capital ranking request completes", async ({ page }) => {
  await page.goto("/");
  await page.getByRole("button", { name: "大型資金建倉" }).click();
  const row = page.getByTestId("stock-row");
  await row.getByRole("button", { name: "加入我的最愛" }).click();
  await expect(row.getByRole("button", { name: "移除我的最愛" })).toHaveAttribute("aria-pressed", "true", { timeout: 300 });
  await expect(row.getByRole("button", { name: "移除我的最愛" })).toHaveAttribute("aria-pressed", "true", { timeout: 3000 });
});
