...
type TargetedScoreJob = { job_id: number; stock_id?: string; status: string; run_mode?: string; target_date?: string; phase?: string; progress?: { completed?: number; total?: number }; started_at?: string; finished_at?: string; datasets?: Record<string, { status?: string; error_code?: string; physical_requests?: number; records_accepted?: number }>; pre_readiness?: Readiness; readiness?: Readiness; score?: { score?: number; status?: string }; fetch_errors?: { dataset?: string; error_code?: string }[]; quota?: { status?: string; remaining?: number; error_code?: string }; error_code?: string };
type Readiness = { stock_id: string; ready: boolean; missing_reasons: string[]; coverage: { readiness_reason_codes?: string[]; missing_sessions?: Record<string, string[]>; holding_missing_weeks?: number[]; RequiredFeatureValidation?: Record<string, { valid?: boolean; reason?: string; expected_window?: number; cadence?: string }> }; source_date: string; knowledge_cutoff?: string };
...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main>
       ...
       {summary?.provider_state?.score_blocked && <div className="error-banner" data-testid="score-blocking-reason"><strong>{reasonLabel[summary.provider_state.reason_code || ""] || summary.provider_state.reason_code || "來源覆蓋率不足"}</strong>　{reasonLabel[summary.provider_state.score_blocking_reason || "SCORE_BLOCKED_BY_SOURCE_COVERAGE"] || "Score 已停止，等待必要來源完成。"}</div>}
